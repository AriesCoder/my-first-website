...
function submitHandler(event){
    ...
    axios.post('http://localhost:5432/link1', body)
    .then(()=>{
        userComment.value = ''
...
function deleteComment(id) {
    axios.delete(`http://localhost:5432/link1/${id}`)
...
function getAllCmtByPage(id){
    ...
    axios.get(`http://localhost:5432/comments/${id}`)
    .then(res => {
      let {username} =  window.localStorage
      let commentCard = ''
...
function getRecipe(){
    ...
    axios.get(`http://localhost:5432/recipes/${name}`)
      .then(res => {
...
function searchRecipes(){
  ...
  result.innerHTML = ''
  let resultCard = ''
  axios.get(`http://localhost:5432/search/${input}`)
  .then(res => { 
...
function getRating(rate){
  ...
  filterCont.innerHTML += "<br><button class='closebutton' onclick='doClose()' >X </button><br>"; 
    axios.get(`http://localhost:5432/link1/${rate}`)
    .then(res => {
      res.data.forEach(elem =>{
